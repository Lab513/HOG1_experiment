...
Training and Validation data are produced on the augmented images using
Scikit learn train_test_split method.
'''
...
def convert_masks(path_masks):
    '''
    Convert the mask from Napari to the good format
    '''
    print('Converting the masks..')
    ll = glob.glob(f'{path_masks}/*.tif')
    print(f'list of masks with tif format is {ll}')
    for l in ll:
        img = io.imread(l)
        new_img = img/img.max()*255
        new_img = cv2.resize(new_img, (512, 512))
        cv2.imwrite(l, new_img.astype('int32'))
    print(f'made format conversion')


class HANDLE(object):

    def __init__(self, *all_dir, kind='train',
                 format='tiff', dim=512, gray=False, dil=0, flood=False):
        '''
        '''
        self.format = format
    ...
    def dilated_contour(self, img):
        ...
        if self.dil > 1:
            ...
            # dilation
            img_out = cv2.dilate(img.copy(), kernel, iterations=1)
        else:
            img_out = img
    ...
    def init_with_rotate(self, angle):
        ...
        # rotate image
        img_r = ie.rotateImage(self.img_orig, angle)
        img = img_r.copy()
        img = ie.random_change(img)
        self.tab_images.append(img)
        # rotate mask
        img_mask = ie.rotateImage(self.img_mask_orig, angle)
    ...
    def specific_flood_fill(self):
        '''
        '''
        # flood fill for the specific training set : "training-cell"
        if self.training_set_name == 'training-cell':
            if self.num in self.l0:
                self.use_flood_from_img_to_mask(self.img_mask_orig,
                                                self.img_orig)

    def init_image_and_mask(self, file, debug=[0,1]):
        '''
        ...
        self.img_mask_orig = self.dilated_contour(
                 self.resize(cv2.imread(str(file_mask), 0)))  # RFP
    ...
    def show_tables(self):
        ...
        '''
        for ident in range(len(self.tab_masks)):
            print(ident)
            cv2.imshow('imageOpen', self.tab_images[ident])
            cv2.imshow('maskOpen', self.tab_masks[ident])
            cv2.waitKey(0)
    ...
    def make_training_and_test_dataset(self):
        ...
        self.tab_masks = np.array(self.tab_masks,
                                  dtype=np.float32)[:, :, :]/255
        self.train_images, self.test_images,\
            self.train_masks, self.test_masks = train_test_split(
                              self.tab_images,
                              self.tab_masks,
                              test_size=0.05)
        del self.tab_images
    ...
    def prepare_images_and_masks(self, activate_ti,
                                 step_ang=60, show_table=False):
        '''
        Perform the transformations on the images before training.
        '''
    ...
    def check_existing_folders_and_files(self):
        '''
        Check if essential folders are there.
        '''
        p = Path('.')
    ...
    def load_test_im(self):
        ...
        self.tab_test_images =\
            np.array(self.tab_test_images, dtype=np.float32)/255  # normalize
