...
class TRANSFORMATIONS():
    ...
    def morph_open(self, iter=3):
        ...
        self.img_mask = cv2.morphologyEx(self.img_mask, cv2.MORPH_OPEN,
                                         kernel, iterations=iter)

    def dilate_mask_shapes(self, dil_size=3, iter=1, debug=[1]):
        '''
        dilate prediction mask
        '''
        if 1 in debug:
            print('Using dilate_mask_shapes !!!')
    ...
    def erode_mask_shapes(self, erd_size=3, iter=1, debug=[1]):
        '''
        erode prediction mask
        '''
        if 1 in debug:
            print('Using erode_mask_shapes !!!')
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,
                                           (erd_size, erd_size))
        #kernel = np.ones((dil_size,dil_size), np.uint8); square kernel
