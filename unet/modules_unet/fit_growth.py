...
class FIT_GROWTH():
    ...
    def growth_curves(self, ax):
        ...
        self.list_growth_rates = []
         # intervals on which are made the fits for the growth rate
        ...
        if not mpl :                       #
            ...
            if self.nb_fits > 0:
                ...
                for i,ll in enumerate(self.list_fits):
                    ...
                    ax.plot(ll[0], ll[1], linestyle='--',
                            label=labelgrowth)    # plot the fitting curves
    ...
    def add_growth_curve(self, mpl, ax):
        ...
        if self.nb_fits > 0:
            # fitting few points after the beginning
            if len(arr_cells[arr_cells > 0]) > 20:
                try:
                    # Fitting exponential curve on BF number of cells extracted..
                    self.make_growth_curves(mpl, ax)
                except:
    ...
    def debug_SG(self):
        ...
        plt.figure()
        # check the derivative for finding the intervalls
        plt.plot(self.diff_cells)
        plt.show()
