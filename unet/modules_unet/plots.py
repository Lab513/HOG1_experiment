...
class PLOTS():
    ...
    def prepare_plots(self, nbcols = 2):
        ...
        print(f"nbcols {nbcols}, nblines {nblines} ")
    ...
    def save_graph_nb_cells(self):
        ...
        '''
        addr_file_nb_cells = self.dir_result / f'nb_cells_curve_{self.name_film}_{self.model}_th{self.thresh_after_pred}_{self.date}.png'
        print(f"#### addr_file_nb_cells {addr_file_nb_cells} ")
        plt.savefig(addr_file_nb_cells)
        self.csv(self.arr_nb_cells)

    def csv(self, array):
        '''
        Save self.curr_obs as csv file
        '''
        addr_csv = self.dir_result / f'nb_cells_curve_{self.name_film}_{self.model}_th{self.thresh_after_pred}_{self.date}.csv'
        df = pd.DataFrame(array)
        df.to_csv(addr_csv, index=False)

    def apply_threshold_for_nbcells(self):
        '''
        '''
        self.img_mask[self.img_mask >= self.thresh_after_pred] = 255          # enhance the prediction mask
        self.img_mask[self.img_mask < self.thresh_after_pred] = 0             # enhance the prediction mask
    ...
    def find_max_nb_cells(self, im):
        ...
        if self.thresh_after_pred:
            self.apply_threshold_for_nbcells()
        try:
            ...
            print('Cannot find contours, set the nb of cells to 0')
            self.max_nb_cells = 0
