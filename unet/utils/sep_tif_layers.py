...
class SEP_TIF_LAYERS(FB):
    ...
    '''
    def __init__(self, addr, temp_dest=None):
        '''
        ...
        self.temp_dest = temp_dest                # folder where to copy the results
    ...
    def make_dir_with_file_name(self):
        ...
        Main folder
        '''
        print(f"folder name for extractions is {self.folder} ")
    ...
    def make_list_bad_pics_withML(self, debug=[2]):
        '''
        '''
        self.find_bad_pics_in_folder(self.folder_layer)               # find indices of bad pics
        self.pics_to_rem = self.ind_bad_pics
        if 2 in debug : print(f' self.pics_to_rem after ML treatment is {self.pics_to_rem}')
    ...
    def make_list_pic_to_rem(self, debug=[1]):
        ...
        '''
        self.pics_to_rem = []
        self.make_list_bad_pics_withML()
        ##
        ll = sorted(list(set(self.pics_to_rem)))                        # sorted list of bad pics
        if 1 in debug :
            print(f' nb of pics removed is {len(ll)}')
            print(f' list ll of pics to be removed is {ll}')
    ...
    def remove_bad_pics_from_layer(self, video, clear, debug=[]):
        ...
        lb = list(set(self.pics_to_rem))                                      # remove doublons
    ...
    def make_list_imgs_for_video_layer(self, debug=[0]):
        '''
        Create the list of images for producing the video
        '''
    ...
    def create_video_layer(self, remove_bad_pics=None, clear=True):
        '''
        Create a video
        For producing movie_cleaned_BF.avi etc..
        '''
    ...
    def cp_to_dest(self):
        '''
        Copy in the temporary destination folder..
        '''
        temp_dest = opj(self.temp_dest, opb(self.folder).replace(' ','_'))
        print(f'After extraction, copy {self.folder} to {temp_dest}')
        sh.copytree(self.folder, temp_dest)
    ...
    def add_tif(self, tif0, tif1):
        ...
        print(f"Address of the merged file is {outp}")
    ...
    def convert_4layers_to_3layers2(self):
        '''
        Convert from (frames, sizex, sizey, layers) to (interwined frames, sizex, sizey)
        '''
        film = io.imread(self.addr)
        part_film = {}
        self.list_layers = range(3)
        for i,layer in enumerate(self.list_layers):
            part_film[i] = film[:,:,:,i]
        s = part_film[0].shape
        img_interm = np.empty((s[0]*3,s[1],s[2]), dtype=film.dtype)
        nblay = 3 #len(self.list_layers)
        for i,layer in enumerate(self.list_layers):
            img_interm[i::nblay] = part_film[i]
        img_final =  img_interm[1::3]
        final_addr = f'{self.addr[:-4]}_3layers.tif'
        print(f'save at address {final_addr}')
        io.imsave(final_addr, img_final)

    def make_composite(self,*prefs, name_comp_vid):
        '''
        Make composite videos from images
        prefs : prefixs for BF, and fluos..
        eg: make_composite(['frame0_t', 'frame0_rfp_t', 'frame0_gfp_t'], name_comp_vid.tif)
        '''
        nblay = len(prefs)                                                      # number of layers
        img = io.imread('frame0_t0.tiff')
        shp = img.shape                                                         # shape of one image, eg (512,512)
        lBF = glob.glob(prefs[0] + '*.tiff')                                  # list of the BF images
        composite = np.empty((len(lBF)*nblay, shp[0], shp[1]))
        print(f'composite.shape is {composite.shape}')
        for i in range(len(lBF)):                                               # go throught the images
            for j in range(nblay):                                              # insert diff kind of images
                name_tiff = f'{prefs[j]}{i}.tiff'
                img = io.imread(name_tiff)
                composite[nblay*i+j,:,:] = img                                  # fill the np.ndarray
        io.imsave(name_comp_vid, composite.astype('uint16'))                   # save the composite video
